...
- Se houver comentário começando com `## Spec` (gerado pela skill spec), ele é o **contrato técnico da task**: endpoints e retornos HTTP, tabelas/relacionamentos, estrutura de pastas e estratégia de testes já estão decididos — o plano deve obedecê-lo, nunca contrariá-lo. Steps de teste seguem exatamente o que a spec definiu (happy path vs. cenários de erro, suites).
- Se não houver comentário `## Spec` mas existir um `spec-<slug>.md` na raiz do projeto referente à task, use-o como a spec (mesma autoridade).
- Se **não** houver spec e a task envolver API/dados/testes não triviais, sugira rodar `/spec <n>` antes — o plano fica melhor com o contrato fechado. Siga sem spec só se o usuário preferir.
...
- Havendo spec (`## Spec` nos comentários), decisões dela são inegociáveis no plano; se durante o planejamento você descobrir que algo da spec não se sustenta, pare e avise em vez de planejar por cima.
